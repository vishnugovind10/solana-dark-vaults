...
- Arcium does not publish a native Windows CLI. The real MXE build and configured two-node local-cluster path run in hosted Linux CI or a contributor's Linux/WSL2 environment.
- Hosted CI verifies `arcium build --skip-keys-sync` and `arcium test` against the local topology. It does not prove a devnet deployment, production recovery set, or end-to-end confidential callback round.
