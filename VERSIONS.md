...
| Arcium CLI / Arx / Arcis / TS SDK | 0.13.2 | Verified in hosted Ubuntu Arcium workflow on commit `951bdc5`: devcontainer, `arcium build --skip-keys-sync`, and `arcium test` passed |

The repository's portable checks (`cargo test --workspace --exclude encrypted-ixs`, coordinator typecheck/tests, and the deterministic demo) remain the baseline CI gate. Arcis macros require the Arcium compiler environment, so the encrypted crate is built only by `arcium build`. The hosted Linux job installs the versions above and passed both Arcium build and local-cluster tests on commit `951bdc5`. Anchor 1.0.2 is intentionally pinned so the CLI can discover the program and `encrypted-ixs` in one Cargo workspace.
